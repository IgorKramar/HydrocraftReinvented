...
**Конвенция снята с 29 готовых пар «базовый / созревший»**, которые в моде
уже нарисованы. Она строже, чем кажется: у фасоли созревший вариант — та же
шпалера с той же листвой плюс стручки; у цветной капусты — та же земля,
на ней четыре кочана ровной сеткой; у финиковой пальмы — та же крона, под ней
гроздья. Кашпо, уровень земли, силуэт куста и масштаб **не меняются вообще**,
добавляется только урожай. Отсюда и сила преобразования для img2img:
**0.3–0.4, не выше**.

**Кадр 200×320 — это пропорция 5:8, портретная.** Генерировать нужно в ней же
(500×800, 640×1024), и не обрезать поля: сверху у растений много прозрачной
пустоты, она часть кадра. При приёмке обязателен `--no-crop`, иначе кадр
пересоберётся по содержимому, кашпо съедет относительно соседних состояний
и грядка дёрнется при созревании:

```sh
python3 tools/make_icon.py import --input помидор.png \
    --out Item_HCPottedtomatoready.png --size 200x320 --no-palette \
    --bg flood --no-crop
```
...
выходило 170–197. `--no-crop` тут не спасает, потому что дело не в обрезке,
а в масштабе. Флаг `--align-to` подгоняет масштаб по ширине основания
и совмещает нижний край с референсом.

*Но выравнивания мало.* Даже после него кашпо оставались разными: у картофеля
самой широкой строкой оказалась листва, а не ящик, и масштаб посчитался по ней
(кромка 97 вместо 109). А доски генератор красит по-своему — базовое дерево
тёплое `#63431d`, у сгенерированных выходило `#402614`, `#4b3328`, `#483021`:
темнее и глуше.

**Первая попытка была неверной.** Я вклеивал в новую картинку нижнюю часть
базовой, отрезая по «самой широкой строке». Но у изометрического ящика самая
широкая строка — **внизу**, а не сверху: разрез рассекал ящик пополам, верх
оставался генераторским, и получалась ступенька с двумя разными деревьями
на одном ящике. Склейка тут вообще не годится.

**Работает подгонка.** `--planter-from` ничего не режет: он находит габариты
деревянной части в обеих картинках, подгоняет под референс масштаб по обеим
осям (для картофеля вышло 0.223×0.189 — по горизонтали и вертикали разный,
иначе ящик остаётся приземистым), совмещает по левому верхнему углу дерева,
а затем приводит цвет досок к референсному переносом статистики: средний тон
и разброс по каждому каналу. Текстура при этом сохраняется, шва нет.

Две тонкости, которые пришлось учесть:

- *Цвет считается только по стенкам*, нижние 35 % деревянной части. Сверху
  тоже есть тёплое и насыщенное — бамбуковые колышки у помидора, земля —
  и от них статистика уезжает в жёлтый: первый заход выкрасил ящик помидора
  в оливковый.
- *Раскладка по крайним тонам не подходит.* Сначала я мапил яркость на
  тройку «тёмный / средний / светлый» из палитры референса — доски уходили
  в темноту. Перенос среднего и разброса даёт куда более естественный
  результат.

```sh
python3 tools/make_icon.py import --input редис.png \
    --out Item_HCPottedradishready.png --bg flood --flood-tol 6 \
    --planter-from Item_HCPottedradish.png
```

`--stretch-v` при этом больше не нужен: приземистость касалась именно ящика,
а он теперь берётся готовым. Флаг остался для случаев, когда основание
взять неоткуда.

С `--align-to` размер холста берётся у референса, поэтому `--size`
и `--no-crop` указывать не нужно.
