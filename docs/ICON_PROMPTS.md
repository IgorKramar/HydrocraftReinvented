...
**Решение — не перерисовывать кашпо вовсе.** Оно во всех состояниях одно
и то же, поэтому берётся из базовой иконки целиком: `--planter-from` вклеивает
всё ниже верхней кромки из референса, а сверху оставляет новое содержимое.
Размер и цвет досок совпадают по построению, сверка даёт ноль различающихся
пикселей в основании. Заодно чинится и замер: ширина основания ищется только
в нижней трети картинки, где листвы уже нет.

*Приземистое кашпо.* Изометрия PZ вытянута по высоте сильнее, чем привыкли
генераторы: у базовой редиски отношение высоты ящика к ширине 0.75, у
сгенерированной — 0.61. Лечится `--stretch-v`; коэффициент считается как
отношение этих чисел, для редиски вышло 1.23 и дало ровно 0.75.

*Порог заливки.* Чем светлее и мягче картинка, тем ниже нужен порог: помидору
и картофелю хватило 10, у редиски при 10 заливка вытекала внутрь и от растения
оставалось 515 пикселей — понадобилось 6.
